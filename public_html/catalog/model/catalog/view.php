...
class ModelCatalogView extends Model {
	...
	public function getView($view_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'images'           => $this->getViewsImage($query->row['view_id']),	
			);
		} else {
			return false;
		}
	...
	public function getVariantImage( $offer_id ) {
		...
		$offers = $this->db->query("
			...
			SELECT DISTINCT(p.image) AS image, o.image AS offer_image FROM " . DB_PREFIX . "variants v
	...
	public function getViewsImage( $view_id ) {
