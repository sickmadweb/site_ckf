...
class ControllerConfiguratorHouse extends Controller {
	private $error = array();
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')/*  && $this->validateForm() */) {
			...
			$this->model_configurator_configurator->addHouse($this->request->post);
			...
			$this->response->redirect($this->url->link('configurator/house', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') /* && $this->validateForm() */) {			
			...
			$this->model_configurator_configurator->editHouse($this->request->get['house_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) /* && $this->validateDelete() */) {
			...
			foreach ($this->request->post['selected'] as $house_id) {
				$this->model_configurator_configurator->deleteHouse($house_id);
			}
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['house_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		...
		if (!isset($this->request->get['house_id'])) {
			$data['action'] = $this->url->link('configurator/house/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('configurator/house/edit', 'user_token=' . $this->session->data['user_token'] . '&house_id=' . $this->request->get['house_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('configurator/house', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['house_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$house_info = $this->model_configurator_configurator->getHouse($this->request->get['house_id']);			
		}
		...
		} elseif (!empty($house_info)) {
			$data['image'] = $house_info['image'];
		} else {
			$data['image'] = '';
		}
		...
		} elseif (!empty($house_info) && is_file(DIR_IMAGE . $house_info['image'])) {
			$data['thumb'] = $this->model_tool_image->resize($house_info['image'], 100, 100);
		} else {
		...
		if (isset($this->request->post['preview'])) {
			...
			$data['preview'] = $house_info['preview'];
		} else {
			$data['preview'] = '';
		}
		// Images
		/* if (isset($this->request->post['product_image'])) {
			...
			$product_images = $this->model_catalog_product->getProductImages($this->request->get['house_id']);
		} else {
		...
		/* if (isset($this->request->post['product_image_aditional'])) {
			...
			$product_aditional_images = $this->model_catalog_product->getProductAditionalImages($this->request->get['house_id']);
		} else {
		...
		$this->response->setOutput($this->load->view('configurator/house_form', $data));
	}
    ...
    protected function getList() {
		...
		*/ 
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('configurator/house', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('configurator/house/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('configurator/house/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$house_total = $this->model_configurator_configurator->getTotalHouses();

		$results = $this->model_configurator_configurator->getHouses($filter_data);
		...
		foreach ($results as $result) {
			...
			$product_specials = $this->model_catalog_product->getProductSpecials($result['house_id']);
			...
			$data['houses'][] = array(
				...
				'edit'       => $this->url->link('configurator/house/edit', 'user_token=' . $this->session->data['user_token'] . '&house_id=' . $result['house_id'] . $url, true)
			);
		}
		...
		*/
		...
		$pagination->total = $house_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('configurator/house', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($house_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($house_total - $this->config->get('config_limit_admin'))) ? $house_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $house_total, ceil($house_total / $this->config->get('config_limit_admin')));
		...
		$this->response->setOutput($this->load->view('configurator/house_list', $data));
	}
