...
class ControllerConfiguratorMaterial extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->document->setTitle($this->language->get('heading_title_materials'));
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')/*  && $this->validateForm() */) {
			...
			$this->model_configurator_configurator->addMaterial($this->request->post);
			...
			$this->response->redirect($this->url->link('configurator/material', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') /* && $this->validateForm() */) {			
			...
			$this->model_configurator_configurator->editMaterial($this->request->get['material_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) /* && $this->validateDelete() */) {
			...
			foreach ($this->request->post['selected'] as $material_id) {
				$this->model_configurator_configurator->deleteMaterial($material_id);
			}
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['material_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title_materials'),
			'href' => $this->url->link('configurator/material', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		if (!isset($this->request->get['material_id'])) {
			$data['action'] = $this->url->link('configurator/material/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('configurator/material/edit', 'user_token=' . $this->session->data['user_token'] . '&material_id=' . $this->request->get['material_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('configurator/material', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['material_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {

			$material_info = $this->model_configurator_configurator->getMaterial($this->request->get['material_id']);  
		}

        $elements = $this->model_configurator_configurator->getElements();
		...
		if (isset($this->request->post['element_id'])) {
			...
			$data['element_id'] = $material_info['element_id'];
		} else {
			$data['element_id'] = '';
		}
		...
		$this->response->setOutput($this->load->view('configurator/material_form', $data));
	}
    ...
    protected function getList() {
		...
		*/ 
		...
		$data['add'] = $this->url->link('configurator/material/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('configurator/material/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$material_total = $this->model_configurator_configurator->getTotalMaterials();

		$results = $this->model_configurator_configurator->getMaterials($filter_data);
		...
		foreach ($results as $result) {				
			...
			$data['materials'][] = array(
				...
				'edit'          => $this->url->link('configurator/material/edit', 'user_token=' . $this->session->data['user_token'] . '&material_id=' . $result['material_id'] . $url, true)
			);
		}
		...
		*/
		...
		$pagination->total = $material_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('configurator/material', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($material_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($material_total - $this->config->get('config_limit_admin'))) ? $material_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $material_total, ceil($material_total / $this->config->get('config_limit_admin')));
		...
		$this->response->setOutput($this->load->view('configurator/material_list', $data));
	}
