...
class ControllerConfiguratorElements extends Controller {
	private $error = array();
	...
	public function index() {
		...
		$this->document->setTitle($this->language->get('heading_title_elements'));
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST')/*  && $this->validateForm() */) {
			...
			$this->model_configurator_configurator->addElement($this->request->post);
			...
			$this->response->redirect($this->url->link('configurator/elements', 'user_token=' . $this->session->data['user_token'] . $url, true));
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') /* && $this->validateForm() */) {			
			...
			$this->model_configurator_configurator->editElement($this->request->get['element_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) /* && $this->validateDelete() */) {
			...
			foreach ($this->request->post['selected'] as $element_id) {
				$this->model_configurator_configurator->deleteElement($element_id);
			}
	...
	protected function getForm() {
		...
		$data['text_form'] = !isset($this->request->get['element_id']) ? $this->language->get('text_add') : $this->language->get('text_edit');
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title_elements'),
			'href' => $this->url->link('catalog/product', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		if (!isset($this->request->get['element_id'])) {
			$data['action'] = $this->url->link('configurator/elements/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('configurator/elements/edit', 'user_token=' . $this->session->data['user_token'] . '&element_id=' . $this->request->get['element_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('configurator/elements', 'user_token=' . $this->session->data['user_token'] . $url, true);

		if (isset($this->request->get['element_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$element_info = $this->model_configurator_configurator->getElement($this->request->get['element_id']);			
		}
		...
		} elseif (!empty($element_info)) {
			$data['name'] = $element_info['name'];
		} else {
			$data['name'] = '';
		}
		...
		$this->response->setOutput($this->load->view('configurator/elements_form', $data));
	}
    ...
    protected function getList() {
		...
		*/ 
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title_elements'),
			'href' => $this->url->link('configurator/elements', 'user_token=' . $this->session->data['user_token'] . $url, true)
		);

		$data['add'] = $this->url->link('configurator/elements/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('configurator/elements/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$element_total = $this->model_configurator_configurator->getTotalElements();

		$results = $this->model_configurator_configurator->getElements($filter_data);
		...
		foreach ($results as $result) {				
			...
			$data['elements'][] = array(
				...
				'edit'       => $this->url->link('configurator/elements/edit', 'user_token=' . $this->session->data['user_token'] . '&element_id=' . $result['element_id'] . $url, true)
			);
		}
		...
		*/
		...
		$pagination->total = $element_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('configurator/elements', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$data['results'] = sprintf($this->language->get('text_pagination'), ($element_total) ? (($page - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page - 1) * $this->config->get('config_limit_admin')) > ($element_total - $this->config->get('config_limit_admin'))) ? $element_total : ((($page - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $element_total, ceil($element_total / $this->config->get('config_limit_admin')));
		...
		$this->response->setOutput($this->load->view('configurator/elements_list', $data));
	}
